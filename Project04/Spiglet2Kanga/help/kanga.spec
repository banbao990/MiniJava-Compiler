...
Kanga is a MIPS-oriented language.  It is a lot like Spiglet, but with the 
following changes:

1) Kanga supports C-style comments.

2) Labels now have global rather than local scope.

3) Instead of an unbounded number of temporaries with local scope,
   Kanga has 24 global machine registers with global scope.  The
...
7) A procedure has three integers in its header, for example:    
   ...
   The first integer has the same meaning as the integer in Spiglet, i.e.
   the number of args taken by the procedure.
